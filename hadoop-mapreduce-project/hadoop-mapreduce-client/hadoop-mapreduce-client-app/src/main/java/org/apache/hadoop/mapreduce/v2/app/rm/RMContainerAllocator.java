    ...
    class ScheduledRequests {
        ...
        private void assignMapsWithLocality(List<Container> allocatedContainers) {
            ...
            while (it.hasNext() && maps.size() > 0 && canAssignMaps()) {
                ...
                if (containerTaskAttemptID != null ) {
                    ...
                    System.out.println("######## TaskAttemptID NULL ######### ");
                }
            }
//        String[] taskIdComponents = taskId.split("_");
//        TaskId reconstructedTaskId = MRBuilderUtils.newTaskId();
//        TaskAttemptId reconstructAttemptId = MRBuilderUtils.newTaskAttemptId();
//        LinkedList<TaskAttemptId> list = mapsHostMapping.get(host);
...
//            }
    ...
    private boolean containerIsLocal(ContainerRequest containerRequest, String allocatedContainerHostName) {
        String[] reqHosts = containerRequest.hosts;

        for (int i = 0; i < reqHosts.length; i++) {
            if (allocatedContainerHostName.equals(reqHosts[i])) {
                return true;
            }
        }
        return false;
    ...
    private boolean containerIsRackLocal(ContainerRequest containerRequest, String allocatedContainerHostName) {
        String[] reqRacks = containerRequest.racks;

        for (int i = 0; i < reqRacks.length; i++) {
            if (allocatedContainerHostName.equals(reqRacks[i])) {
                return true;
            }
        }
        return false;
